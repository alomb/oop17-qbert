...
public class PointComponentImpl implements PointComponent {
     ...
     */
    public PointComponentImpl(final int score) {
        this.points = score;
        this.lifeThreshold = PointComponentImpl.INITIAL_LIFE_THRESHOLD;
        while (this.lifeThreshold <= this.points) {
            this.lifeThreshold += PointComponentImpl.STANDARD_LIFE_THRESHOLD;
        }
    }
    ...
    public void score(final int amount, final Player qbert) {
        ...
        if (points - amount < lifeThreshold && points >= lifeThreshold) {
            lifeThreshold += PointComponentImpl.STANDARD_LIFE_THRESHOLD;
            qbert.gainLife();
        }
    }
